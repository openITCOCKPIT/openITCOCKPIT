...
/** @var \itnovum\openITCOCKPIT\ConfigGenerator\Statusengine3Cfg $Statusengine3Cfg */
...
<form ng-submit="submit();" class="form-horizontal">
    ...
    <?php foreach ($Statusengine3Cfg->getDefaults()['int'] as $key => $defaultValue): ?>
        <div class="form-group required" ng-class="{'has-error': errors.Configfile.<?php echo $key; ?>}">
            ...
            <div class="help-block">
                <?php echo h($Statusengine3Cfg->getHelpText($key)); ?>
            </div>
        </div>
    <?php endforeach; ?>

    <?php foreach ($Statusengine3Cfg->getDefaults()['string'] as $key => $defaultValue): ?>
        <div class="form-group required" ng-class="{'has-error': errors.Configfile.<?php echo $key; ?>}">
            <label class="control-label">
                <?php echo h($key); ?>
            </label>

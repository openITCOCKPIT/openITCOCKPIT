 ...
 */
class NagiosXmlNotificationCommand extends Command {
    ...
    public function buildOptionParser(ConsoleOptionParser $parser): ConsoleOptionParser {
        ...
        $parser->addOptions([
            ...
            'hostdescription'   => ['help' => __d('oitc_console', 'Host description you want to send a notification'), 'default' => ''],
            'hoststate'         => ['help' => __d('oitc_console', 'current host state'), 'default' => ''],
            'hostaddress'       => ['help' => __d('oitc_console', 'host address'), 'default' => ''],
            'hostoutput'        => ['help' => __d('oitc_console', 'host output'), 'default' => ''],
            'hostlongoutput'    => ['help' => __d('oitc_console', 'host long output'), 'default' => ''],
            'hostackauthor'     => ['help' => __d('oitc_console', 'host acknowledgement author'), 'default' => ''],
            'hostackcomment'    => ['help' => __d('oitc_console', 'host acknowledgement comment'), 'default' => ''],
            'contactmail'       => ['help' => __d('oitc_console', 'recivers mail address'), 'default' => ''],
            'contactalias'      => ['help' => __d('oitc_console', 'human name of the contact'), 'default' => ''],
            'servicedesc'       => ['help' => __d('oitc_console', 'Service uuid you want to notify'), 'default' => ''],
            'servicestate'      => ['help' => __d('oitc_console', 'service state'), 'default' => ''],
            'serviceoutput'     => ['help' => __d('oitc_console', 'service output'), 'default' => ''],
            'servicelongoutput' => ['help' => __d('oitc_console', 'service long output'), 'default' => ''],
            'serviceackauthor'  => ['help' => __d('oitc_console', 'service acknowledgement author'), 'default' => ''],
            'serviceackcomment' => ['help' => __d('oitc_console', 'service acknowledgement comment'), 'default' => ''],
            ...
            'layout'            => ['help' => __d('oitc_console', 'E-Mail layout/template that should be used'), 'default' => 'notification'],
        ]);
    ...
    public function execute(Arguments $args, ConsoleIo $io) {
        ...
        // Even if the Acknowledgement via E-Mail has been removed, some Customers are may use a self written solution on Python or similar
    ...
    private function sendHostNotification(Host $Host, Arguments $args) {
        ...
        $Mailer->viewBuilder()
            ->setTemplate('xml_' . $this->layout . '_' . $this->type)
    ...
    private function validateOptions(Arguments $args) {
        ...
        if ($args->getOption('layout') !== 'default') {
            ...
            $xmlTemplateExists = false;


            $filename = sprintf('%s_%s.php', $layout, $this->type);

            if (file_exists(APP . 'Template' . DS . 'email' . DS . 'text' . DS . 'xml_' . $filename)) {
                $xmlTemplateExists = true;
            }

            if ($xmlTemplateExists) {
                $this->layout = $layout;
            } else {
                Log::error('Notification: XML layout "' . $layout . '" not found. Using default.');
            }
